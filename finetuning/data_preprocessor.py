...
"""
Data preprocessing script for finetuning datasets.

This script downloads, formats, and aggregates multiple datasets (Alpaca, Tulu V2, UltraChat)
into a unified JSONL format for training and testing.
"""
...
def format_alpaca(example):
    """
    Formats an Alpaca example into the standard instruction format.

    Args:
        example (dict): An example from the Alpaca dataset.

    Returns:
        dict: A dictionary with 'instruction', 'input', and 'output' keys.
    """
    return {
...
def format_tulu(example):
    """
    Formats Tulu V2 examples into instruction format.
    
    Tulu structure: 'messages': [{'role': 'user', 'content': '...'}, {'role': 'assistant', 'content': '...'}]
    System prompts are prepended to the user instruction.

    Args:
        example (dict): An example from the Tulu V2 dataset.
    ...
    # Prepend system prompt to user instruction if present
    user_content = []
    assistant_content = []
    ...
    # Target the final assistant response
    if assistant_content:
        output_text = assistant_content[-1]
...
def format_ultrachat(example):
    """
    Formats UltraChat examples into instruction format.
    
    Extracts the first user message as instruction and the last assistant message as output.

    Args:
        example (dict): An example from the UltraChat dataset.
...
def process_dataset(source_name, dataset_path, split, format_func, samples_train, samples_test):
    """
    Downloads, shuffles, formats, and splits a dataset.

    Args:
        source_name (str): Human-readable name of the dataset source.
        dataset_path (str): Hugging Face dataset path.
        split (str): Dataset split to load.
        format_func (callable): Function to format dataset examples.
        samples_train (int): Number of training samples to select.
        samples_test (int): Number of test samples to select.

    Returns:
        tuple: A tuple containing (train_data, test_data) lists.
    """
